...
@brief get an value proportional to the summation of the pixels inside a rectangular region
@param areg 		target AREG
@param r0 			start row address of the region
@param c0 			start column address of the region
@param r1 			last row address of the region
@param c1 			last column address of the region
@return the result

When the size of the rectangular is 64x64, the returned value can be ranged between
[0,255] when the pixel inside it has a range of [-128,127].
*/
uint8_t scamp5_global_sum_region(const areg_t& areg,uint8_t r0,uint8_t c0,uint8_t r1,uint8_t c1);
