...
*/
void scamp5_scan_events(DSRC dreg,uint8_t*cr2v,uint16_t max_num=1,uint8_t h_dir=0,uint8_t v_dir=0);


/*!
	\brief same as ::scamp5_scan_events, but scan for row address first.

	\param dreg 		DREG
	\param rc2v 		pointer to a buffer of (max_num*2) bytes
	\param max_num		maximum number of events
	\param h_dir		horizontal order of scanning, '0' means from column 0 to column 255
	\param v_dir		vertical order of scanning, '0' means from row 0 to row 255

	This function scan for row address first, and the first byte in the buffer is row address.
*/
void scamp5_scan_events_rc(DSRC dr,uint8_t*rc2v,uint16_t max_num=1,uint8_t h_dir=0,uint8_t v_dir=0);

void scamp5_scan_events_cr(DSRC dr,uint8_t*rc2v,uint16_t max_num=1,uint8_t h_dir=0,uint8_t v_dir=0);
...
/*!
	...
	E.g. when \p s is 2, the image size will be 128x128.
...
#define SCAMP5_MAX_AER_ITERATIONS	4000
...
void scamp5_output_io_begin();
void scamp5_output_io_end();
